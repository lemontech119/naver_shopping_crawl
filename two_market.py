...
def get_products_from_naver_order_by_review_count(base_url, last_paging):
    ...
    for paging_num in range(1, last_paging+1):
        print(paging_num)
...
def main():
    base_url = "https://search.shopping.naver.com/search/all?frm=NVSHCHK&origQuery=%EB%A7%88%EC%BC%93%EC%BB%AC%EB%A6%AC&pagingSize=40&productSet=checkout&query=%EB%A7%88%EC%BC%93%EC%BB%AC%EB%A6%AC&sort=review&timestamp=&viewType=list&"
    
    last_paging = 311
    f = open("대표님 요청사항(마켓컬리).csv", "a")
    f.write("대분류,중분류,소분류,세분류,상품명,가격,리뷰수,등록일,판매처,배송비,URL\n")
    f.close()
